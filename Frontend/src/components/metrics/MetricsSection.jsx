...
export function MetricsSection({ metrics }) {
  ...
  // Show only the first valid entry for each dashboard.
  const displayMetrics = safeMetrics.filter(
    (metric, index, arr) =>
      metric?.dashboard_name &&
      arr.findIndex(
        (item) => item?.dashboard_name === metric.dashboard_name
      ) === index
  );
    ...
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-8 duration-700 delay-100 fill-mode-both">
      ...
      <div className="grid lg:grid-cols-2 gap-6 relative">
        {displayMetrics.map((metric, index) => (
          <DashboardCard
            key={`${metric.dashboard_name}-${index}`}
            metric={metric}
            index={index}
          />
        ))}
